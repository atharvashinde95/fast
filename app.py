...
# ─────────────────────────────────────────────
# Startup config & connectivity check
# ─────────────────────────────────────────────
_cfg = validate_config()
if not _cfg["ok"]:
    st.error("⚠️ **Configuration / connectivity error — the app cannot reach the LLM.**")
    for err in _cfg["errors"]:
        st.error(f"• {err}")
    st.info(
        "**How to fix:**\n"
        "1. Make sure you are connected to the **Capgemini VPN**.\n"
        "2. Check your `.env` file exists in the project folder with:\n"
        "   ```\n"
        ...
        "   ```\n"
        "3. Run `python check_connection.py` for a full diagnostic.\n"
        "4. Restart the app after fixing."
    )
    st.stop()   # Halt rendering — don't show broken chat UI

for warn in _cfg.get("warnings", []):
    st.warning(f"⚠️ {warn}")
    ...
    if user_text:
        ...
        with st.spinner("Processing…"):
            try:
                ...
                err_msg = str(exc)
                if "getaddrinfo" in err_msg or "ConnectionError" in err_msg or "ConnectError" in err_msg:
                    friendly = (
                        "❌ **Network error:** Cannot reach the LLM endpoint.\n\n"
                        "Check your VPN connection and try again.\n\n"
                        f"_Details: `{err_msg[:200]}`_"
                    )
                else:
                    friendly = f"❌ **Unexpected error:** `{err_msg[:300]}`"
